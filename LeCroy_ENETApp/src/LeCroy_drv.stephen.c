...
static void LeCroyRecoverLinkThread( void *parm)
LeCroyID	LeCroy_Open(char * ipaddr, int channels, BOOL auto_link_recover)
STATUS	LeCroy_Get_LinkStat(LeCroyID lecroyid, int *plinkstat)
STATUS	LeCroy_Get_Model(LeCroyID lecroyid, char * model)
STATUS	LeCroy_Get_IPAddr(LeCroyID lecroyid, char * ipaddr)
...
static const iocshArg * const vipcArgs[1] =
	{&vipcArg0};
static const iocshFuncDef vipc664FuncDef =
	{"ipacAddVIPC664", 1, vipcArgs};
static void vipc664CallFunc(const iocshArgBuf *args) {
	ipacAddVIPC664(args[0].sval);
}
static void epicsShareAPI vipc664Registrar(void) {
	iocshRegister(&vipc664FuncDef, vipc664CallFunc);
}
epicsExportRegistrar(vipc664Registrar);
